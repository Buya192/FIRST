...
const MilestoneBaik: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoadingData(true);
    try {
      const querySnapshot = await getDocs(collection(db, 'milestoneBaik'));
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      if (editingItem) {
        const docRef = doc(db, 'milestoneBaik', editingItem.id);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const docRef = doc(db, 'milestoneBaik', id);
  ...
  return (
    <Card title="Milestone Baik" className="p-6">
