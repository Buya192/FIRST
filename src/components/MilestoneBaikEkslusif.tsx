...
const MilestoneBaikEkslusif: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoadingData(true);
    try {
      const querySnapshot = await getDocs(collection(db, 'milestoneBaikEksklusif'));
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      if (editingItem) {
        const docRef = doc(db, 'milestoneBaikEksklusif', editingItem.id);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const docRef = doc(db, 'milestoneBaikEksklusif', id);
  ...
  return (
    <Card title="Milestone Baik Ekslusif" className="p-6">
...
export default MilestoneBaikEkslusif;