...
import { Table, Card, Button, Upload, Input, Tooltip, Form, Modal, message, InputNumber, Select } from 'antd';
...
const MilestoneRusakEkslusif: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoadingData(true);
    try {
      const querySnapshot = await getDocs(collection(db, 'milestoneRusakEksklusif'));
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      if (editingItem) {
        const docRef = doc(db, 'milestoneRusakEksklusif', editingItem.id);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const docRef = doc(db, 'milestoneRusakEksklusif', id);
  ...
  const columns = [
    ...
    { title: 'Daya (kVA)', dataIndex: 'daya', key: 'daya', width: 120 },
    {
      title: 'Foto 1',
      dataIndex: 'foto1',
      key: 'foto1',
  ...
  return (
    <Card title="Milestone Rusak Ekslusif" className="p-6">
...
export default MilestoneRusakEkslusif;